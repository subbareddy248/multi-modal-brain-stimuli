...
def load_tvlt(story):
    ...
    if story=='bourn':
        ...
        for i in np.arange(10):
            if i+1<10:
                with open('features_representations/'+modality+'_embeddings_bourne0'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data['bourne0'+str(i+1)])])
            else:
                with open('features_representations/'+modality+'_embeddings_bourne'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data['bourne'+str(i+1)])[:379,:]])
        train_data.append(np.hstack(temp_data))
    elif story=='wolf':
        temp_data = []
        for i in np.arange(17):
            if i+1<10:
                with open('features_representations/'+modality+'_embeddings_wolf0'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data['wolf0'+str(i+1)])[:,:]])
            else:
                with open('features_representations/'+modality+'_embeddings_wolf'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data['wolf'+str(i+1)])[:,:]])
        ...
        for i in np.arange(10):
            if i+1<10:
                with open('features_representations/'+modality+'_embeddings_bourne0'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([sess_data])
            else:
                with open('features_representations/'+modality+'_embeddings_bourne'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data)[:379,:]])
        train_data.append(np.hstack(temp_data))
        temp_data = []
        for i in np.arange(17):
            if i+1<10:
...
def load_test_tvlt():
    ...
    for i in np.arange(5):
        with open('features_representations/'+modality+'_embeddings_life0'+str(i+1)+'.pkl', 'rb') as f:
            sess_data = pickle.load(f)
        temp_data.append([np.array(sess_data)])
...
def load_meta(story):
    ...
    if story=='bourn':
        ...
        for i in np.arange(10):
            if i+1<10:
                with open('../MetaTransformer/meta_features_movie10/'+modality+'_embeddings_bourne0'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data)])
            else:
                with open('../MetaTransformer/meta_features_movie10/'+modality+'_embeddings_bourne'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data)[:379,:]])
        train_data.append(np.hstack(temp_data))
    elif story=='wolf':
        ...
        for i in np.arange(17):
            if i+1<10:
                with open('../MetaTransformer/meta_features_movie10/'+modality+'_embeddings_wolf0'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data)[:,:]])
            else:
                with open('../MetaTransformer/meta_features_movie10/'+modality+'_embeddings_wolf'+str(i+1)+'.pkl', 'rb') as f:
                    sess_data = pickle.load(f)
                temp_data.append([np.array(sess_data)[:,:]])
        train_data.append(np.hstack(temp_data)[:,:6989])
    elif story=='all':
...
def load_test_meta():
    ...
    temp_data = []
    for i in np.arange(5):
        with open('../MetaTransformer/meta_features_movie10/'+modality+'_embeddings_life0'+str(i+1)+'.pkl', 'rb') as f:
            sess_data = pickle.load(f)
        temp_data.append([np.array(sess_data)])
    test_data.append(np.hstack(temp_data)[0,:2008,:])
    return test_data[0]
...
def load_test_videomae(modality, eachlayer):
    test_data = []
    temp_data = []
...
if __name__ == "__main__":
    ...
    Rstim_video, Pstim_video = load_data('video', args.story), load_test_data('video')
    Rstim_audio, Pstim_audio = load_data('audio', args.story), load_test_data('audio')
    #Rstim, Pstim = load_meta(args.story), load_test_meta()
    #Rstim, Pstim = load_videomae(args.modality, args.story, 0), load_test_videomae(args.modality, 0)
    #print(np.array(Rstim).shape, np.array(Pstim).shape)

    # Rstim = np.random.rand(11013,768)
    # Pstim = np.random.rand(2008,768)
    Rstim = np.nan_to_num(zscore(np.concatenate([Rstim_video,Rstim_audio],axis=1)))
    Pstim = np.nan_to_num(zscore(np.concatenate([Pstim_video,Pstim_audio],axis=1)))
    ...
    for eachlayer in np.arange(1):
        ...
        all_corrs = []
        if not os.path.exists(save_dir+'/'+subdir+'/'+args.modality+'/'+args.story):
            os.makedirs(save_dir+'/'+subdir+'/'+args.modality+'/'+args.story)
        wt, corr, alphas, bscorrs, valinds = bootstrap_ridge(np.nan_to_num(delRstim[eachlayer]), np.nan_to_num(zRresp), np.nan_to_num(delPstim[eachlayer]), np.nan_to_num(zPresp),
        ...
        np.save(os.path.join(save_dir+'/'+subdir+'/'+args.modality+'/'+args.story, "layer_"+str(eachlayer)),voxcorrs)